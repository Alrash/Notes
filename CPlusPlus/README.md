...
同vector、list、stack等，属于基本容器，用于存储类似（键值，键值对应的值）这样的数据（l类似元组），简单说起来就是一个学号对应一个人一样，这里学号就是键值，具体的那个人就是其对应的值。（一对一关系（つ＞ω●）つ）
...
例如：原本有(0,0),(1,0)这两对在map内，若插入(2,0)，也可插入成功<br>
**注意，map一个键无法对应多个值，但是，可以插入成功**
例如，原本有(0,0)这样的对应关系，当我们想要插入(0,1)时，会得到插入成功，键0对应值为1，而不是0
